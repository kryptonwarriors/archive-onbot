...
@Autonomous(name = "Blue Full", group = "")
public class BlueFull extends LinearOpMode {
    ...
    if (opModeIsActive()) {
      ...
      if (!targetVisible) {
        ...
        runtime.reset();
        while (opModeIsActive() && (!(targetVisible) && runtime.seconds() < 1) && !isStopRequested()){
          for (VuforiaTrackable trackable : allTrackables) {
      ...
      sleep(400);
      //Park
      DistanceUsingRightDist(LEFT, 35, 0.4, 2);
